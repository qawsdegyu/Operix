...
// Use a permissive CORS policy to avoid issues on Vercel preview URLs or local testing.
// Since rate limiting is applied, the API is protected from basic abuse.
...
app.post('/api/ai/chat', chatLimiter, async (req, res) => {
  try {
    ...
    console.error("Chat Error:", error); // full details logged server-side only
    if (!res.headersSent) {
...
app.use((err, req, res, next) => {
  console.error(err.stack); // log full details server-side
